...
maxtimeout = 60 * 60  # Set the maximum timeout value to 1 hour (60 minutes * 60 seconds)
...
async def pex_fetch_allpages(url,i=1):    
    ...
    if is_instance(pex_page_id_noid,str):
...
async def pex_fetch_allpages(url,i=1):
    base_url = pex_remove_page_string(url)
    pex_page_id_noid= await get_pex_fileid(url,True)    
    if isinstance(pex_page_id_noid, str):
        total_pages = int(await pex_get_pagenum(url))
        has_downloaded = False
        if has_downloaded is False:
            await  pex_fetch_somepages(url,i,total_pages)
...
async def fetch(url):
    ...
    retry = 0
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=maxtimeout)) as session:
        while retry < url_get_maxretries_nu:            
            ...
            if response.status == 200:
                ...
                # If the response was unsuccessful, try again
                retry += 1
                continue
    await session.close()
